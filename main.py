...
def create_candlestick_chart(df, ticker, timeframe='1d'):  
    ...
    try:  
        ...
        timeframe_text = "Weekly" if timeframe == '1w' else "Daily"  

        fig, axlist = mpf.plot(  
            df_plot,  
            type='candle',  
            title=f'\n{ticker} Stock Price ({timeframe_text})',  
            volume=True,  
            style=s,  
            figsize=(15, 10),  
            returnfig=True  
...
def fetch_stock_data(ticker, timeframe='1d', start_date=None, end_date=None):  
    """  
    Fetch stock price data  
    timeframe: '1d' for daily, '1w' for weekly  
    ...
    try:  
        ...
        # Resample to weekly data if requested  
        if timeframe == '1w':  
...
async def get_stock_data(  
    ...
    """  
    Get stock data with specified timeframe and start date  
    ticker: stock symbol  
    timeframe: '1d' for daily, '1w' for weekly  
    ...
    if timeframe not in ['1d', '1w']:  
        raise HTTPException(status_code=400, detail="Invalid timeframe. Use '1d' for daily or '1w' for weekly")  

    try:  
        # Parse start_date if provided, otherwise use 2 years ago  
        if start_date:  
        ...
        df, error = fetch_stock_data(ticker, timeframe, start_date=start, end_date=end_date)  
        if error:  
        ...
        if df is not None and not df.empty:  
            # Generate chart  
            chart_image = create_candlestick_chart(df, ticker, timeframe)  
            if chart_image:  
